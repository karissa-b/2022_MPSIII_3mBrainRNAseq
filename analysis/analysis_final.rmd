...
fc.ab <- function(x){exp(rnorm(x.ab, log(1), 0.5*log(2)))}
...
fc.ac <- function(x){exp(rnorm(x.ac, log(1), 0.5*log(2)))}

dim(x.ac$counts)

ac.powerplot <- ssizeRNA_vary(nGenes = 16073, # Num detectable genes
              pi0 = 0.9, # Proportion of non-DE genes
...
fc.bc <- function(x){exp(rnorm(x.bc, log(1), 0.5*log(2)))}

dim(x.bc$counts)

bcPowerPlot <- ssizeRNA_vary(nGenes = 16073, # Num detectable genes
              pi0 = 0.9, # Proportion of non-DE genes
              m = 20, # pseudo sample size
...
acpp <- ac.powerplot$power %>% 
  as_tibble() %>% 
  set_colnames(c("Sample size (n)", "Power")) %>% 
  ggplot(aes(x = `Sample size (n)`, 
             y = Power)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(limits = c(0,20))+
  scale_y_continuous(limits = c(0,1)) +
  geom_hline(yintercept =  0.6620218) +
  geom_vline(xintercept =  7) +
  annotate(geom = "label", x = 1.5, y = 0.75, label = "~66% power\n n = 7") +
  ggtitle("A and C")

bcpp <- bcPowerPlot$power %>% 
  as_tibble() %>% 
  set_colnames(c("Sample size (n)", "Power")) %>% 
  ggplot(aes(x = `Sample size (n)`, 
             y = Power)) +
  ...
  scale_x_continuous(limits = c(0,20))+
  scale_y_continuous(limits = c(0,1)) +
  geom_hline(yintercept =  0.5251222) +
  geom_vline(xintercept =  11) +
  annotate(geom = "label", x = 1.5, y = 0.6, label = "~50% power\n n = 11") +
  ggtitle("B and C")

abpp <- abPowerPlot$power %>% 
  as_tibble() %>% 
  set_colnames(c("Sample size (n)", "Power")) %>% 
  ggplot(aes(x = `Sample size (n)`, 
             y = Power)) +
  ...
  scale_x_continuous(limits = c(0,20))+
  scale_y_continuous(limits = c(0,1)) +
  geom_hline(yintercept =  0.2496993) +
  geom_vline(xintercept =  4) +
  annotate(geom = "label", x = 1.5, y = 0.4, label = "~25% power\n n = 4") +
  ggtitle("A and B")
