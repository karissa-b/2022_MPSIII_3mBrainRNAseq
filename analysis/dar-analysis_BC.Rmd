---
title: "DAR Analysis"
subtitle: "2022_MPSIII_3mBrainRNAseq dataset - BC comparison"
author: "Lachlan Baer"
...
```{r topTables}
topTables <- readRDS(here("data/R_objects/toptab_bc_cqn.rds"))
```
...
The *hgnsat* gene implicated with MPS-IIIC is located on chromosome 1.
...
This dataset provides an interesting scenario where samples were genotyped at both the *naglu* and *hgsnat* loci.
Samples within the MPS-IIIB experimental groups were selected so that they were wildtype at the *hgsnat* locus.
...
An analogous situation is present in the MPS-IIIC samples, where the samples should share near-identical wildtype homologues of chromosome 24.
...
```{r darECDF_C_chr24}
plotDarECDF(dar$C_vs_WT, darVal = "region", highlight = "24") +
  scale_colour_manual(values = c("red", "grey")) +
  labs(title = "MPSIIIC vs WT")
```

Again we see relatively high DAR on the mutant chromosome (1), and relatively low DAR on the wildtype controlled chromosome (24).
