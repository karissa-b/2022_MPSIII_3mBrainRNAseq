...
This document contains the Differential Allelic Representation (DAR) analysis for the MPS-III B/C arm of the experiment.
...
This metadata contains information for all samples in the experiment, so let's filter for only samples in the B/C dataset.
...
```{r cumDAR_MPSIIIB}
plotCumDAR(DAR$B_vs_WT, "24", "MPS-IIIB vs. WT")
```
...
```{r winRanges_MPSIIIB}
plotWinRanges(winRanges$B_vs_WT, title = "MPS-IIIB vs. WT")
```
